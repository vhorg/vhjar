...
public class ToolStationContainer extends OverSizedSlotContainer {
   private final ToolStationTileEntity tileEntity;
   private final BlockPos tilePos;

   public ToolStationContainer(int windowId, Level world, BlockPos pos, Inventory playerInventory) {
      super(ModContainers.TOOL_STATION_CONTAINER, windowId, playerInventory.player);
      this.tilePos = pos;
      if (world.getBlockEntity(this.tilePos) instanceof ToolStationTileEntity craftingStationTileEntity) {
   ...
   private void initSlots(Inventory playerInventory) {
      for (int row = 0; row < 3; row++) {
         for (int column = 0; column < 9; column++) {
            this.addSlot(new TabSlot(playerInventory, column + row * 9 + 9, 8 + column * 18, 90 + row * 18));
         }
      }

      for (int hotbarSlot = 0; hotbarSlot < 9; hotbarSlot++) {
         this.addSlot(new TabSlot(playerInventory, hotbarSlot, 8 + hotbarSlot * 18, 148));
      }
      ...
      this.addSlot(new RecipeOutputSlot(this.tileEntity.getResultContainer(), 0, 148, 39));
   }
   ...
   public ToolStationTileEntity getTileEntity() {
