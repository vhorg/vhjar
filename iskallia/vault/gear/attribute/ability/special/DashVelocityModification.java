...
public class DashVelocityModification extends FloatValueAbilityModification<FloatValueConfig> {
   public static final ResourceLocation ID = VaultMod.id("dash_velocity");

   public DashVelocityModification() {
      super(ID);
   }

   public float adjustVelocity(FloatValueConfig config, float velocity) {
      return velocity * (1.0F + config.getValue());
   }

   public static SpecialAbilityGearAttribute.SpecialAbilityConfig<?, ?> newConfig(float value) {
      return new SpecialAbilityGearAttribute.SpecialAbilityConfig<>("Dash", ID, new FloatValueConfig(value));
   }
   ...
   public MutableComponent getDisplay(FloatValueConfig config, Style style, VaultGearModifier.AffixType type) {
      ...
      return valueDisplay == null
         ...
         : new TextComponent("")
            ...
            .append(" to ")
            .append(new TextComponent("Dash").withStyle(getAbilityStyle()))
            .append(" Velocity");
   }
