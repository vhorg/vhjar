...
public class ShockedParticleMessage {
   ...
   public static ShockedParticleMessage decode(FriendlyByteBuf buffer) {
      return new ShockedParticleMessage(
   ...
   public static void handle(ShockedParticleMessage message, Supplier<Context> contextSupplier) {
         ...
         if (level.getEntity(entityID) instanceof LivingEntity livingEntity) {
            for (int i = 0; i < 20; i++) {
               if (pe.createParticle(
                  (ParticleOptions)ModParticles.SHOCKED.get(),
                  pos.x(),
                  pos.y(),
                  pos.z(),
