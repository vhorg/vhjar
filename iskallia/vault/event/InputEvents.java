...
public class InputEvents {
   ...
   private static void onInput(Minecraft minecraft, int key, int action) {
      if (minecraft.field_71462_r == null && key != -1) {
         for (Entry<String, KeyBinding> quickSelectKeybind : ModKeybinds.abilityQuickfireKey.entrySet()) {
            if (quickSelectKeybind.getValue().getKey().func_197937_c() == key) {
               if (action != 1) {
                  return;
               }
         ...
         if (ModKeybinds.abilityWheelKey.getKey().func_197937_c() == key) {
            if (action != 1) {
               return;
            }
         ...
         } else if (ModKeybinds.openShardTraderScreen.func_151468_f()) {
            ModNetwork.CHANNEL.sendToServer(new ShardTraderScreenMessage());
         } else if (ModKeybinds.openAbilityTree.func_151468_f()) {
            ModNetwork.CHANNEL.sendToServer(new OpenSkillTreeMessage());
         } else if (ModKeybinds.abilityKey.getKey().func_197937_c() == key) {
            if (action == 0) {
