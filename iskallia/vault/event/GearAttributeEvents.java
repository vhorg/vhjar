...
public class GearAttributeEvents {
   ...
   public static void blockAttack(LivingAttackEvent event) {
      ...
      if (!attacked.getLevel().isClientSide() && !event.getSource().isBypassInvul()) {
         boolean hasShield = attacked.getItemInHand(InteractionHand.MAIN_HAND).getItem() instanceof ShieldItem;
         hasShield |= attacked.getItemInHand(InteractionHand.OFF_HAND).getItem() instanceof ShieldItem;
         if (hasShield) {
