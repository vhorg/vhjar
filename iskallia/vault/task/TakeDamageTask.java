...
public class TakeDamageTask extends ProgressConfiguredTask<Float, TakeDamageTask.Config> {
   public TakeDamageTask() {
      super(new TakeDamageTask.Config(), TaskCounter.Adapter.FLOAT);
   }

   public TakeDamageTask(TakeDamageTask.Config config, TaskCounter<Float, ?> counter) {
      ...
      CommonEvents.ENTITY_HURT.register(this, EventPriority.HIGHEST, event -> {
         if (this.parent == null || this.parent.hasActiveChildren()) {
            if (this.getConfig().phase == DamagePhase.PRE_MITIGATION) {
               this.onTakeDamage(context, event.getSource().getEntity(), event.getEntity(), event.getAmount());
            }
         }
      });
   ...
   public void onTakeDamage(TaskContext context, Entity attacker, Entity attacked, float amount) {
      if (context.getSource() instanceof EntityTaskSource source) {
         if (attacked != null && !attacked.getLevel().isClientSide()) {
            if (attacker == null || attacker.getLevel() == attacked.getLevel()) {
               if (source.matches(attacked)) {
                  if (attacker == null || this.getConfig().filter.test(attacker)) {
