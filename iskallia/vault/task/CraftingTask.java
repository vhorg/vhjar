...
public class CraftingTask extends ProgressConfiguredTask<Integer, CraftingTask.Config> implements IProgressTask {
   public CraftingTask() {
      super(new CraftingTask.Config(), 0, Adapters.INT_SEGMENTED_7, Integer::compare);
   }
         ...
         if (source instanceof EntityTaskSource entitySource) {
            if (entitySource.matches(event.getPlayer())) {
               if (this.getConfig().filter.test(event.getCrafting())) {
                  Integer var4 = this.currentCount;
   ...
   public void onDetach() {
      CommonEvents.PLAYER_CRAFT.release(this);
      super.onDetach();
   }
   ...
   public static class Config extends ConfiguredTask.Config {
      ...
      public Config(ItemPredicate filter, IntRoll count) {
         this.filter = filter;
         this.count = count;
      }
