...
public class ArchitectEventGenerator extends VaultGenerator {
   ...
   public boolean generate(ServerLevel world, VaultRaid vault, MutableBlockPos pos) {
      BoundingBox box = vault.getProperties().getBase(VaultRaid.BOUNDING_BOX).orElseGet(() -> {
         ...
         return new BoundingBox(min.getX(), 0, min.getZ(), max.getX(), 256, max.getZ() + 8192);
      });
      ...
      try {
         ...
         StructureStart start = ((ArchitectEventStructure.Feature)ModFeatures.ARCHITECT_EVENT_FEATURE.value())
         ...
         world.getChunk(chunkPos.x, chunkPos.z, ChunkStatus.EMPTY, true)
            .setStartForFeature((ConfiguredStructureFeature)ModFeatures.ARCHITECT_EVENT_FEATURE.value(), start);
