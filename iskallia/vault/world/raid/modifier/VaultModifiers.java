...
public class VaultModifiers implements INBTSerializable<CompoundNBT> {
   ...
   public void generate(Random random, int level, boolean raffle) {
      this.modifiers.addAll(ModConfigs.VAULT_MODIFIERS.getRandom(VaultRarity.values()[this.raid.rarity], random, level, raffle));
   }
   ...
   public CompoundNBT serializeNBT() {
      ...
      this.modifiers.forEach(group -> modifiersList.add(StringNBT.func_229705_a_(group.getName())));
   ...
   public void deserializeNBT(CompoundNBT nbt) {
      ...
      ListNBT modifiersList = nbt.func_150295_c("Modifiers", 8);

      for (int i = 0; i < modifiersList.size(); i++) {
         this.modifiers.add(ModConfigs.VAULT_MODIFIERS.getByName(modifiersList.func_150307_f(i)));
      }
   }

   public void encode(PacketBuffer buffer) {
      buffer.writeInt(this.modifiers.size());
      this.modifiers.forEach(group -> buffer.func_180714_a(group.getName()));
   }
   ...
   public static VaultModifiers decode(PacketBuffer buffer) {
      ...
      for (int count = buffer.readInt(); i < count; i++) {
         res.modifiers.add(ModConfigs.VAULT_MODIFIERS.getByName(buffer.func_218666_n()));
      }
