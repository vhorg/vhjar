...
public class NonGrantedCrystalShapedRecipe extends ShapedRecipe {
   static int MAX_WIDTH = 3;
   static int MAX_HEIGHT = 3;

   public NonGrantedCrystalShapedRecipe(
   ...
   public static class Serializer extends ForgeRegistryEntry<RecipeSerializer<?>> implements RecipeSerializer<ShapedRecipe> {
      ...
      private static String[] patternFromJson(JsonArray jsonArr) {
         ...
         if (astring.length > NonGrantedCrystalShapedRecipe.MAX_HEIGHT) {
            throw new JsonSyntaxException("Invalid pattern: too many rows, " + NonGrantedCrystalShapedRecipe.MAX_HEIGHT + " is maximum");
         } else if (astring.length == 0) {
            throw new JsonSyntaxException("Invalid pattern: empty pattern not allowed");
         } else {
            for (int i = 0; i < astring.length; i++) {
               String s = GsonHelper.convertToString(jsonArr.get(i), "pattern[" + i + "]");
               if (s.length() > NonGrantedCrystalShapedRecipe.MAX_WIDTH) {
                  throw new JsonSyntaxException("Invalid pattern: too many columns, " + NonGrantedCrystalShapedRecipe.MAX_WIDTH + " is maximum");
               }
