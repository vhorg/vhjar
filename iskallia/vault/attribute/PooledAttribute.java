...
public abstract class PooledAttribute<T> extends VAttribute.Instance<T> {
   ...
   public abstract static class Generator<T, O extends PooledAttribute.Generator.Operator<T>> implements VAttribute.Instance.Generator<T> {
      @Expose
   ...
   public static class Pool<T, O extends PooledAttribute.Pool.Operator<T>> {
      ...
      public List<T> getEntries() {
         return this.entries.stream().map(entry -> entry.value).collect(Collectors.toList());
      }
