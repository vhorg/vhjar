...
public class ImmortalityIndicatorRenderer {
   public static void render(Entity entity, PoseStack poseStack, MultiBufferSource bufferSource, Quaternion cameraOrientation) {
      if (entity instanceof LivingEntity livingEntity) {
         MobEffectInstance effectInstance = livingEntity.getEffect(ModEffects.IMMORTALITY);
         if (effectInstance != null) {
            poseStack.pushPose();
            poseStack.translate(0.0, entity.getBbHeight() + 0.5, 0.0);
            poseStack.mulPose(cameraOrientation);
            poseStack.scale(-0.025F, -0.025F, 0.025F);
            VertexConsumer buffer = bufferSource.getBuffer(ModRenderTypes.IMMORTALITY_INDICATOR);
