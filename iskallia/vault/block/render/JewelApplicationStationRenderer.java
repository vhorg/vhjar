...
public class JewelApplicationStationRenderer implements BlockEntityRenderer<VaultJewelApplicationStationTileEntity> {
   public JewelApplicationStationRenderer(Context context) {
   }

   public void render(
      VaultJewelApplicationStationTileEntity tile,
      float partialTicks,
      ...
      if (!tool.isEmpty()) {
         ...
         minecraft.getItemRenderer().renderStatic(tool, TransformType.FIXED, combinedLightIn, combinedOverlayIn, poseStack, bufferIn, 0);
         poseStack.popPose();
      }
   }
}
