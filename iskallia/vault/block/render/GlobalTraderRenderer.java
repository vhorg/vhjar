...
public class GlobalTraderRenderer extends TileEntityRenderer<GlobalTraderTileEntity> {
   ...
   public void render(
      GlobalTraderTileEntity tileEntity, float partialTicks, MatrixStack matrixStack, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay
   ) {
      BlockState blockState = tileEntity.func_195044_w();
      ResourceLocation skinLocation = tileEntity.getSkin().getLocationSkin();
      this.renderTrader(matrixStack, blockState, buffer.getBuffer(PLAYER_MODEL.func_228282_a_(skinLocation)), combinedLight, combinedOverlay, 1.0F);
